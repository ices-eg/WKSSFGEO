...
```
The code flow is driven by:
 * Solve "problems" at root, not downstream
 * Delay filtering of data to the last moment possible
 * Wrap things that are non-intuative or repetative into functions - use
    desriptive name for function (verbs, desribing what it does)
```

```
Should we aim at using standard variable names:
  * vid: vessel id
  * gid: gear id
  * hit: harbour id
  * tid: trip id
...
rb_trip_identify <- function(d, vid = vid, hid = hid) {
  d %>% 
...
**The barbour is not in a proper format, hence**:
...
1. Identify points in harbour, hence use {sf}-functions
2. Calculate consecutive points statistics by vessel and trip id
...
d <- 
  ...
  # Get the harbour id for each point
  st_join(hb) %>% 
  st_drop_geometry() %>% 
  rb_trip_identify() %>% 
  # ----------- problem, should not keep in final pipe flow, solve at root
  group_by(vid, tid) %>% 
  mutate(n.pings = n()) %>% 
  ungroup() %>% 
  filter(n.pings >= 3) %>% 
...
**Some thoughts on summary statistics** (could also be done by trip if added to `group_by`):
...
**Abnormal step distances** within a trip (note value of dist in km). Shown are
also adjacent values (hence us of `rb_peek` function:

```{r}
d %>% 
  rb_peek(dist, 5000) %>% 
  select(rid:dura, hid, speed2) %>% 
