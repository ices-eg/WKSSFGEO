...
subtitle: "data-examples/example_data_AIS.csv"
...
hb <- 
  ...
  select(hid)
d <- 
  read_csv("data-examples/example_data_AIS.csv") %>% 
...
Note that for some vessels we have very short trips (few pings), may warrant some further investigartion into why (or possibly check for code bugs). Note also that vessel 5 is the whole time in harbour, and does little. Need to take these things into account in the downstream coding.
...
d <- 
  ...
  mutate(n = n()) %>% 
  #     step speed (nmi)
  mutate(ss = ifelse(n > 2 & tid > 0 & vid != 5,
  ...
  mutate(whacky = ifelse(n >= 6 & tid > 0 & vid != 5,
...
d %>% 
  filter(tid > 0, vid == 11, tid %in% 10:21) %>%
  # just for visuals
  mutate(ss = ifelse(ss > 30, 30, ss),
         whacky = ifelse(whacky == "removed", TRUE, FALSE),
         size = ifelse(whacky, 1, 0.01)) %>% 
  ggplot(aes(time)) +
  theme_bw() +
  geom_point(aes(y = speed), size = 1, colour = "grey") +
  geom_point(aes(y = ss, colour = as_factor(whacky), size = size),
             alpha = 0.5) +
  scale_size_area(max_size = 2) +
  facet_wrap(~ tid, scales = "free_x", ncol = 3) +
  scale_y_log10() +
...
... decision on what to do with whacky points
